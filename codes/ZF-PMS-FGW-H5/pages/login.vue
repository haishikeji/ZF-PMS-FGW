<template>
	<view class="normal-login-container">
		<view class="logo-content align-center justify-center flex">
			<text class="title">项目调度管理系统</text>
		</view>
		<view class="login-form-content">
			...
			<view class="rember">
				<uni-data-checkbox multiple v-model="checkbox" :localdata="change"></uni-data-checkbox>
			</view>
		...
		methods: {
			...
			loginSuccess(result) {
				...
				this.$store.dispatch('GetInfo').then(res => {
					let name = res.user.roles[0].roleKey
					console.log(res.user.roles[0].roleKey)
					this.$tab.reLaunch('/pages/index')
					// return
					// // 根据用户权限进入不同页面
					// if (name == 'xtadmin' || name == 'xzadmin') {
					// 	this.$tab.reLaunch('/pages/leader')
					// } else if (name == 'ptjs' || name == '王敬清') {
					// 	this.$tab.reLaunch('/pages/index')
					// } else {
					// 	uni.showToast({
					// 		title: '用户权限有误',
